...
std::optional<Uint8Array> png_get_exif_data(const std::string png_data) {
  ...
  if (png_get_eXIf_1(png_ptr, info_ptr, &exif_data_len, &exif_data_ptr) != 0 &&
    ...
    auto *output = static_cast<unsigned char *>(
        malloc(sizeof(ExifHeader) + exif_data_len));
    if (output == nullptr) {
      png_destroy_read_struct(&png_ptr, &info_ptr, nullptr);
      return std::nullopt;
    }

    memcpy(output, ExifHeader, sizeof(ExifHeader));
    memcpy(output + sizeof(ExifHeader), exif_data_ptr, exif_data_len);
    png_destroy_read_struct(&png_ptr, &info_ptr, nullptr);
    return std::optional<Uint8Array>{Uint8Array(
        val(typed_memory_view(sizeof(ExifHeader) + exif_data_len, output)))};
  }
#endif
...
Uint8Array png_set_exif_data(const std::string png_data,
...
#ifdef PNG_eXIf_SUPPORTED
  bool hasExifHeader =
      strcmp(exif_data.data(), reinterpret_cast<const char *>(ExifHeader)) == 0;
  auto png_exif_data = reinterpret_cast<png_bytep>(
      const_cast<char *>(hasExifHeader ? exif_data.data() + sizeof(ExifHeader)
                                       : exif_data.data()));
  auto png_exif_data_size = static_cast<png_uint_32>(
      hasExifHeader ? exif_data.size() - sizeof(ExifHeader) : exif_data.size());
  ...
  png_destroy_read_struct(&read_png_ptr, &info_ptr, NULL);
  png_destroy_write_struct(&write_png_ptr, NULL);
